...
FluentDialog {
    ...
    body: [
        // 类别：支出/收入分页
        FluentSegmented {
            visible: control.manageCategories
            Layout.fillWidth: true
            labelA: qsTr("支出类别")
            labelB: qsTr("收入类别")
            value: control.catType
            onEdited: function(newValue) {
                control.catType = newValue
                control.editingId = -1
                control.revision++
            }
        },
            ...
            ListView {
                ...
                delegate: Rectangle {
                    ...
                    RowLayout {
                        ...
                        visible: control.editingId !== entityRow.modelData.id
                        ...
                        FluentButton {
                            ...
                            rightPadding: 10
                            onClicked: {
                                control.editingId = entityRow.modelData.id
                                control.renameText = entityRow.modelData.name
                            }
                        }
                        FluentButton {
                            text: qsTr("删除")
                            ...
                            leftPadding: 10
                            rightPadding: 10
                            onClicked: control.doDelete(entityRow.modelData.id)
                        }
                    }
                    ...
                    RowLayout {
                        ...
                        FluentTextField {
                            Layout.fillWidth: true
                            font.pixelSize: 13
                            text: control.renameText
                            onTextEdited: control.renameText = text
                        }
                        FluentButton {
                            text: qsTr("确定")
                            implicitHeight: 26
        ...
        RowLayout {
            ...
            FluentTextField {
                id: newNameField
                Layout.fillWidth: true
                placeholderText: control.manageCategories
                                 ? qsTr("新类别名称") : qsTr("新付款方式名称")
                onAccepted: control.addEntity()
            }
            FluentButton {
                text: qsTr("添加")
                onClicked: control.addEntity()
            }
        },
